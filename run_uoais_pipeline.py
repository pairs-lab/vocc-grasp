...
def compute_summary(results: list[dict], skipped: list[dict], requested: int) -> dict:
    buckets = defaultdict(lambda: {
        ...
        "candidate_F1": [],
        "free_F1": [],
        # occlusion-path reasoning (ported from run_uoais_pipeline_real.py)
        "OP": [],
        "OR": [],
        "OF1": [],
        "MP_NED": [],
...
def _depth_delta(edge: dict) -> float:
    ...
    A raw NaN silently corrupts both the direction resolver (``tanh(NaN)`` -> NaN
    makes every reciprocal-pair comparison False, so the *wrong* direction is
    kept) and the depth-rescue gate. Treating a missing depth delta as 0 (no
    depth evidence either way) lets ``_explained_frac`` decide the direction,
    which is the correct behaviour when depth is unreliable.
    """
...
# (ported from run_uoais_pipeline_real.py: linear blend, no sigmoid)
...
def run(args) -> None:
    ...
    with pred_path.open("w", encoding="utf-8") as pred_f, results_jsonl.open("w", encoding="utf-8") as res_f, zipfile.ZipFile(META_ZIP) as zmeta:
        ...
        for image_rank, image_id in enumerate(sorted(by_image), start=1):
            ...
            # Cached graphs written before calib_conf existed lack the field, and
            # ``select_for_target`` copies edge dicts, so re-annotate in memory here
            # (idempotent) before any selection reads them.
            annotate_calib_conf(graph_record.get("edges_all", []))
            annotate_calib_conf(graph_record.get("edges", []))
